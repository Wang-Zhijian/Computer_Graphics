...
class MyCanvas(QGraphicsView):
    ...
    def save_canvas(self, filename, width, height):
        painter = QPainter()
    ...
    def start_rotate(self):
        ...
        self.temp_item = self.item_dict[self.selected_id]
        self.pList = self.temp_item.p_list
        x_coordinate, xCoordinatePressed = QInputDialog.getInt(self, '旋转', '中心点x坐标:', QLineEdit.Normal)
        y_coordinate, yCoordinatePressed = QInputDialog.getInt(self, '旋转', '中心点y坐标:', QLineEdit.Normal)
        angle, anglePressed = QInputDialog.getInt(self, '旋转', '旋转角度:', QLineEdit.Normal)
    ...
    def start_scale(self):
        ...
        self.temp_item = self.item_dict[self.selected_id]
        self.pList = self.temp_item.p_list
        x_coordinate, xCoordinatePressed = QInputDialog.getInt(self, '缩放', '中心点x坐标:', QLineEdit.Normal)
        y_coordinate, yCoordinatePressed = QInputDialog.getInt(self, '缩放', '中心点y坐标:', QLineEdit.Normal)
        multiple, multiplePressed = QInputDialog.getDouble(self, '缩放', '缩放倍数:', QLineEdit.Normal)
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        y = int(pos.y())
        if self.status == 'line':
            self.temp_item = MyItem(self.temp_id, self.status, [[x, y], [x, y]], self.temp_algorithm, self.main_window.color)
...
class MainWindow(QMainWindow):
    ...
    def reset_canvas_action(self):
        ...
        errorFlag = 0
        while True:
            if (height < 1 or width < 1) and (errorFlag == 0):
                height, heightPressed = QInputDialog.getInt(self, '重置画布', '高:', QLineEdit.Normal)
                width, widthPressed = QInputDialog.getInt(self, '重置画布', '宽:', QLineEdit.Normal)
                errorFlag = 1
            elif (height < 1 or width < 1) and (errorFlag == 1):
                QMessageBox.critical(self, '错误', '输入非法')
                errorFlag = 0
            else:
                break
    ...
    def save_canvas_action(self):
        ...
        dialog=QFileDialog()
        filename=dialog.getSaveFileName(filter="*.bmp")
        if filename[0]:
